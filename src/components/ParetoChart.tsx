...
export default function ParetoChart({ riskItems }: ParetoChartProps) {
  ...
  const limitedRisks = displayMode === 'all' 
    ? sortedRisks 
  ...
  const data = limitedRisks.map((item) => {
    ...
    return {
      name: (item.risk_catalog_base?.name || item.custom_risk_name || 'Rischio').substring(0, 15) + ((item.risk_catalog_base?.name || '').length > 15 ? '...' : ''),
  ...
  if (sortedRisks.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Analisi di Pareto</h3>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
          <button
            ...
            className={`px-3 py-1 text-sm font-medium rounded-md transition ${
              displayMode === '10' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-600 hover:text-gray-800'
            }`}
          >
            Top 10
          </button>
          <button
            ...
            className={`px-3 py-1 text-sm font-medium rounded-md transition ${
              displayMode === '20' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-600 hover:text-gray-800'
            }`}
          >
            Top 20
          </button>
          <button
            ...
            className={`px-3 py-1 text-sm font-medium rounded-md transition ${
              displayMode === 'all' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-600 hover:text-gray-800'
            }`}
          >
      ...
      <div className="h-80 overflow-x-auto">
        <div style={{ minWidth: Math.max(500, data.length * 50) }}>
